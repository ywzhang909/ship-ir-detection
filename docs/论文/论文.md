...
![图11](figures/detection/det_T7_jitter.png)

图 11 空域预处理（Top-hat+CLAHE，YOLO11m+Fusion）在真实 NSLSR 验证帧上的检测结果：检出 31 个舰艇目标，置信度阈值 0.05。

![图12](figures/detection/det_starnet.png)

图 12 频域预处理（Butterworth+CLAHE）在真实 NSLSR 验证帧上的检测结果：检出 21 个舰艇目标，与空域方法相比检出数量较少但误检也较少。

![图13](figures/detection/det_T4_wavelet.png)

图 13 小波 DWT 子带缩放（db4 基，LL×0.3/LH·HL×1.5/HH×0.3）在真实 NSLSR 验证帧上的检测结果：检出 22 个舰艇目标，小波方法兼顾多分辨率子带。
...
![图14](figures/detection/det_T16_crrp.png)

图 14 CRRP 增强模型（YOLO11l+Fusion+CRRP）在真实 NSLSR 验证帧上的检测结果：检出 6 个高置信度舰艇目标，为全方法中置信度最高的配置，误检最少。
...
Run1 验证集 mAP50 为 0.9396、mAP50-95 为 0.6366，测试集 mAP50 为 92.66%、mAP50-95 为 62.35%，测试 Precision 94.84%、Recall 88.81%。Run2 验证集 mAP50 为 0.9465、mAP50-95 为 0.6433，测试集 mAP50 为 93.97%、mAP50-95 为 63.75%，测试 Precision 96.17%、Recall 90.54%。T4 测试集 mAP50 为 97.45%、mAP50-95 为 66.10%，验证-测试差距 +2.0。图 11 至图 13 分别给出三种预处理方法在真实 NSLSR 验证帧上的检测结果：空域 Top-hat（Run1）检出 31 个目标，频域 Butterworth（Run2）检出 21 个目标，小波 DWT（T4）检出 22 个目标。
...
![图15](figures/detection/det_baseline.png)

图 15 YOLO11m + Fusion 基线模型在真实 NSLSR 验证帧上的检测结果：检出 18 个舰艇目标，置信度阈值 0.05，整体置信度分布较为集中。

![图16](figures/detection/det_T1_fusion.png)

图 16 T1 大融合架构（h=32、layers=3、CBAM）在真实 NSLSR 验证帧上的检测结果：检出 51 个目标，大量低置信度误检，验证了该架构在小训练集上严重过拟合。

![图17](figures/detection/det_T2_fusion.png)

图 17 T2 残差搜索（residual=0.7）在真实 NSLSR 验证帧上的检测结果：检出 51 个目标，置信度分布与 T1 相近，两者均因过拟合导致大量误检。

![图18](figures/detection/det_T5_yolo11l.png)

图 18 T5 YOLO11l + Fusion 在真实 NSLSR 验证帧上的检测结果：检出 20 个目标，置信度显著高于 T1/T2，骨干容量升级有效修复过拟合。
...
![图19](figures/detection/det_T20_dyn.png)

图 19 T20 动态检测头在真实 NSLSR 验证帧上的检测结果：检出 19 个舰艇目标，置信度分布合理，未出现显著误检。

![图20](figures/detection/det_T22_large.png)

图 20 T22 大模型（YOLO11l 扩大参数）在真实 NSLSR 验证帧上的检测结果：检出 21 个舰艇目标，整体表现稳定。
...
P2 小目标检测头在 batch=16、8、4 下均显存溢出（OOM），总显存需求约 18 至 22GB，超出 RTX 5080 Laptop 16GB，方向取消。FFA-Net 去雾、LAFI-Diffusion 生成数据与 TensorRT INT8 量化尚未实现。图 19 和图 20 分别展示 T20 动态头与 T22 大模型在真实 NSLSR 验证帧上的检测结果：T20 动态检测头检出 19 个目标，T22 大模型（YOLO11l 扩大参数）检出 21 个目标，两者置信度分布均较为合理，未出现 T1/T2 的大量低置信度误检现象。本文局限在于伪标签本身为估计值，目标宽高由视角估计，标注误差可能限制精度上限；后续可在真实标注数据上验证结论。
